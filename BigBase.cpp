...
BigBase::BigBase(const this_type& refer) { copy(refer); }

BigBase::BigBase(const char* str) : BigBase(std::string(str)) {}
...
void BigBase::aly_hex(const std::string str) { init_hex(str); }
...
BigBase::this_type& BigBase::operator+=(const this_type& integer) {
  if (length >= integer.length && length > 0) {
    ...
    for (; i < integer.length; i++) {
      uint64_t tmp =
          (uint64_t)(blocks[i]) + (uint64_t)(integer.blocks[i]) + carry;
      blocks[i] = (uint32_t)(tmp);
      carry = tmp >> 32;
    }
...
BigBase::this_type& BigBase::operator-=(const this_type& integer) {
  if (length >= integer.length && length > 0) {
    ...
    for (; i < integer.length; i++) {
      uint64_t tmp =
          (uint64_t)(blocks[i]) - (uint64_t)(integer.blocks[i]) - borrow;
      blocks[i] = (uint32_t)(tmp);
      borrow = (uint32_t)(tmp >> 32) & 1;
    }
...
bool BigBase::iszero() const { return length == 0; }
...
size_t BigBase::_length() const { return length; }
...
void BigBase::divideWithRemainder(const this_type& integer, bool quotient,
                                  uint32_t* quotient_array) {
  if (length < integer.length) {
    if (quotient) memset(blocks, 0, sizeof(uint32_t) * length);
  } else {
    int l1 = length - 1;
    ...
    for (int i = l1; i >= 0 && i >= l2; i--) {
      ...
      if ((i == l2) && ((c_gap < 0) || (c_gap == 0 && t1 < t2))) continue;
      ...
      for (int j = i - l2 - g, k = 0; j <= l1 && k <= (l2 + g); j++, k++) {
        uint32_t s = 0;
        if (k <= l2) s = div_shift(integer.blocks[k], c_gap, g);
...
static uint32_t to_int9(std::string& str, size_t start, size_t count) {
  uint32_t c = 0;
  for (size_t i = start; i < str.length() && i < count + start; i++) {
    c *= 10;
    c += str[i] - '0';
  }
  return c;
